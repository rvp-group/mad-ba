#include "se3_pose_pose_right_chordal_error_factor.h"
#include "srrg_solver/solver_core/error_factor_impl.cpp"
#include "srrg_solver/solver_core/instance_macros.h"

namespace srrg2_solver {
  void SE3PosePoseChordalEulerRightErrorFactor::errorAndJacobian(bool error_only_) {
    using JacobianMatrixType = Eigen::Matrix<Scalar, 12, 6>;

    // ia take objects - static cast for speeeeeed
    VariableSE3EulerRight* v_from = _variables.at<0>();
    VariableSE3EulerRight* v_to   = _variables.at<1>();
    assert(v_from && v_to &&
  ...
  void SE3PosePoseChordalEulerRightErrorFactor::_drawImpl(ViewerCanvasPtr canvas_) const {
    if (!canvas_) {
      throw std::runtime_error("SE3PosePoseChordalEulerErrorFactor::draw|invalid canvas");
    }
    Vector3f coords[2];
    coords[0] = static_cast<const VariableSE3EulerRight*>(variable(0))->estimate().translation();
    coords[1] = static_cast<const VariableSE3EulerRight*>(variable(1))->estimate().translation();
    canvas_->pushColor();
  ...
  INSTANTIATE(SE3PosePoseChordalEulerRightErrorFactor)
