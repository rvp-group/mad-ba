...
namespace srrg2_solver {
  ...
  struct SE3SensorExtrinsicPoseMotionCalibAD
    ...
    ADErrorVectorType operator()(VariableTupleType& vars) {
      ...
      TransformTypeAD prediction_ad    = offset_ad * _measure_ad * offset_ad.inverse();
      TransformTypeAD error_SE3        = (_inv_from_ad * _to_ad).inverse() * prediction_ad;
    ...
    void setMeasurement(const MeasurementType& measurement_) override {
      ...
      convertMatrix(_measure_ad, _measurement);
    }
    ...
    TransformTypeAD _measure_ad = TransformTypeAD::Identity();
