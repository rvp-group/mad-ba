...
namespace srrg2_solver {
  ...
  void SE2PosePointBearingErrorFactor::_drawImpl(srrg2_core::ViewerCanvasPtr canvas) const {
    if (!canvas) {
      return;
    }
    ...
    const VariablePoint2* to_v     = dynamic_cast<const VariablePoint2*>(variable(1));
    if (!from_v || !to_v) {
      return;
    }

    const Isometry2f& from = from_v->estimate();
    const Vector2f& to     = to_v->estimate();
    segments[0] << from.translation().x(), from.translation().y(), 0.f;
    segments[1] << to.x(), to.y(), 0.f;
    canvas->pushColor();
