...
namespace srrg2_solver {
  ...
  bool SE3EpipolarBAOffsetLeftFactor_<VariableType_>::_resetCompute(bool chi_only) {
    _is_valid=true;
    _H_adder.reset();
    _b_adder.reset();
    _chi_adder.reset();
    ...
    const auto& var_i = this->variables().template at<0>();    // platform pose from
    const auto& var_j = this->variables().template at<1>();    // platform pose to
    const auto& offset_i = this->variables().template at<2>(); // offset_from
    const auto& offset_j = this->variables().template at<3>(); // offset_to
    if (offset_i->status()!=VariableBase::Fixed) {
      throw std::runtime_error(std::string(__PRETTY_FUNCTION__)+ "offset_from  not fixed");
    }
    if (offset_j->status()!=VariableBase::Fixed) {
      throw std::runtime_error(std::string(__PRETTY_FUNCTION__)+ "offset_to  not fixed");
    }
    
    const auto& Xi               = var_i->estimate()*offset_i->estimate();
    const auto& Xj               = var_j->estimate()*offset_j->estimate();
  ...
  template <typename VariableType_>
  void SE3EpipolarBAOffsetLeftFactor_<VariableType_>::compute(bool chi_only, bool force) {
  ...
  template <typename VariableType_>
  void SE3EpipolarBAOffsetLeftFactor_<VariableType_>::finalizeCompute(bool chi_only){
    if (chi_only)
      return;
    //h_ii
  ...
  template <typename VariableType_>
  void SE3EpipolarBAOffsetLeftFactor_<VariableType_>::errorAndJacobian(float& e,
