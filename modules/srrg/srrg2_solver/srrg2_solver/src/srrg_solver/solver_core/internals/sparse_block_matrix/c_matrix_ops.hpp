...
// Product A * B
inline int cMatMatProd(float* dest,
                              const float* src1, int nr1, int nc1,
                              const float* src2, int nr2, int nc2) {
  if (nc1 != nr2) {
    return -1;
  }
...
// Product A^T * B
static inline int cMatTransposeMatProduct(float* dest,
                             const float* src1, int nr1, int nc1,
                             const float* src2, int nr2, int nc2){
...
inline int cMatTranspose(float* const __restrict__ dest_,
...
inline int cMatSolveTriangular(float* const x,
                                      const float* a,
...
inline int cMatInvert(float* const __restrict__ dest,
                      const float* const __restrict__ src_,
                      const int n) {
...
inline int cMatCopy(float* const __restrict__ dest,
...
inline void cMatPrint(std::ostream& os, const float* src, int nr, int nc) {
  os << "MAT r: " << nr << ", c: " << nc << ", values:" << "\n[";
  for (int r = 0; r < nr; ++r) {
    const float* s = src + r;
    os << "  ";
    for (int c = 0; c < nc; ++c, s += nr) {
...
inline int cMatTransposeInPlace(float* const __restrict__ src, const int n) {
  // loop over columns
  for (int c = 0; c < n; ++c) {
    for (int r = c + 1; r < n; ++r) {
      float ap = src[n * c + r];
...
inline int cMatLLt(float* const __restrict__ src, const int n) {
  // loop over columns
  for (int c = 0; c < n; ++c) {
    // col array
    float* c_ptr = src + c * n;
...
inline int cMatScaleInPlace(float* const __restrict__ dest,
...
inline int cMatVecProdSub(float* const __restrict__ dest,
...
inline int cMatMatLeftProdInPlace(float* const __restrict__ self,
...
int cMatMatRightProdInPlace(float* const __restrict__ self,
                            const float* __restrict__ other,
                            const int nr, const int nc) {
  float* dest = self;
  int dim = nr * nc;
