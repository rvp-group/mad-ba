...
namespace srrg2_solver {
  using namespace std;
  static constexpr int SPARSE_BLOCK_CHOLESKY_TRANSPOSITION_BUFFER_SIZE=4096;
  ...
  bool SparseBlockCholesky::blockBackwardSubstitution(IntBlockMap& x) const {
    ...
    float invL_cc_workspace[max_size];
    for(int c = blockCols()-1; c>=0; --c){
      ...
      MatrixBlockAlias invL_cc(transpose_invL_cc->rows(), transpose_invL_cc->cols(), invL_cc_workspace);
  ...
  bool SparseBlockCholesky::forwardSubstitute(float* x) const {
    ...
    float buffer[SPARSE_BLOCK_CHOLESKY_TRANSPOSITION_BUFFER_SIZE];
    ...
    for (int r = 0; r < blockCols(); ++r) {
      ...
      for (auto it = row.begin(); it != row.end(); ++it) {
        ...
        assert(_l->size() < SPARSE_BLOCK_CHOLESKY_TRANSPOSITION_BUFFER_SIZE && "temporary buffer overflow");
