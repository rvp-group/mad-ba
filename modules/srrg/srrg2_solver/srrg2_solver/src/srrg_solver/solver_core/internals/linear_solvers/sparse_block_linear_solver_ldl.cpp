...
namespace srrg2_solver {
  ...
  SparseBlockLinearSolverLDL::SparseBlockLinearSolverLDL() {
    ...
    // _cholmodCommon.final_super        = 0;
    _cholmodCommon.final_ll           = 0;
  }
  ...
  void SparseBlockLinearSolverLDL::_cacheInverseDiagonal() {
    if (!_L) {
      return;
    }
    cholmod_change_factor(CHOLMOD_REAL, 1, 0, 1, 1, _L, &_cholmodCommon);
    assert(_L->is_ll && !_L->is_super && _L->is_monotonic &&
           "SparseBlockLinearSolverLDL::_cacheInverseDiagonal()|Cholesky factor has wrong "
           "format");
    ...
    assert(cholesky_factor_entries != nullptr &&
           "SparseBlockLinearSolverLDL::_cacheInverseDiagonal()| cholmod_factor coefficients "
           "pointer is void");

    assert(column_pointers != nullptr &&
           "SparseBlockLinearSolverLDL::_cacheInverseDiagonal()| cholmod_factor column "
           "pointers empty");
    ...
    for (int c = 0; c < n; ++c) {
      ...
      assert(inv_diag == inv_diag &&
             "SparseBlockLinearSolverLDL::_cacheInverseDiagonal| nan entry in diagonal");
      _inverse_diagonal[c] = inv_diag;
    }
  }

  SparseBlockLinearSolver::Status SparseBlockLinearSolverLDL::updateStructure() {
    ...
    assert(num_cols == 1 &&
           "SparseBlockLinearSolverLDL::updateStructure| b vector should have one column");
    ...
    if (_cholmodCommon.status != CHOLMOD_OK || _L == nullptr) {
      return SparseBlockLinearSolver::StructureBad;
    }
  ...
  SparseBlockLinearSolver::Status SparseBlockLinearSolverLDL::updateCoefficients() {
    ...
    if (_cholmodCommon.status == CHOLMOD_NOT_POSDEF) {
      std::cerr << "SparseBlockLinearSolverLDL::updateCoefficients| Matrix is not positive definite"
                << std::endl;
    }
  ...
  SparseBlockLinearSolver::Status SparseBlockLinearSolverLDL::updateSolution() {
    ...
    _solution = cholmod_solve(CHOLMOD_LDLt, _L, _c, &_cholmodCommon);
    //std::cerr << "Conditioned number : " << cholmod_rcond(_L, &_cholmodCommon) << std::endl;
  ...
  void SparseBlockLinearSolverLDL::computeOrderingHint(std::vector<int>& ordering,
  ...
  double SparseBlockLinearSolverLDL::_computeMatrixEntry(const int& r, const int& c) {
    assert(r <= c && "SparseBlockLinearSolverLDL::_computeMatrixEntry()| r > c");
    int idx                                = r * _A->rows() + c;
    ...
    assert(inv_Lrr == inv_Lrr &&
           "SparseBlockLinearSolverLDL::_computeMatrixEntry| inverse diagonal element is nan");
    if (r == c) {
  ...
  bool SparseBlockLinearSolverLDL::computeBlockInverse(SparseBlockMatrix& inverse_blocks,
                                                       const std::vector<IntPair>& blocks_layout) {
    _cacheInverseDiagonal();
    if (_inverse_diagonal.empty()) {
      std::cerr << "SparseBlockLinearSolverLDL::computeBlockInverse| cache for inverse "
      ...
      for (int r = 0; r < block_dimensions.first; ++r) {
        for (int c = 0; c < block_dimensions.second; ++c) {
          ...
          assert(it != _elements_cache.end() &&
                 "SparseBlockLinearSolverLDL::computeBlockInverse| something broken in the "
                 "cached elements");
