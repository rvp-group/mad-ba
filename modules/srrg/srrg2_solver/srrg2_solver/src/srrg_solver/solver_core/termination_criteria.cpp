...
namespace srrg2_solver {
  ...
  bool SimpleTerminationCriteria::hasToStop() const {
    // 1st iteration in a level always good
    int current_iteration = _solver->currentIteration();
    if (_solver->levelChanged() || current_iteration < 2) {
      return false;
    }
  ...
  bool RelativeGradientChiTerminationCriteria::hasToStop() const {
    ...
    float stabilized_error_norm = 1 + std::sqrt(_solver->iterationStats().back().chi_kernelized);
  ...
  bool RelativeGradientChiAndConstraintViolationTerminationCriteria::hasToStop() const {
    std::vector<float> gradient;
    getRHS(gradient);
    float squared_norm_gradient = 0;
    for (const float& x_i : gradient) {
    ...
    float stabilized_error_norm = 1 + std::sqrt(_solver->iterationStats().back().chi_kernelized);
    float gradient_norm         = std::sqrt(squared_norm_gradient);
    if (gradient_norm > param_epsilon_gradient.value() * stabilized_error_norm) {
      return false;
    }
    if (_solver->iterationStats().back().constraint_violation_norm >
        param_epsilon_violation.value()) {
      return false;
    }
    return true;
  }

  bool PerturbationNormAndConstraintViolationTerminationCriteria::hasToStop() const {
    std::vector<float> perturbation;
    getPerturbation(perturbation);
    float squared_norm = 0;
    for (const float& x_i : perturbation) {
    ...
    if (_solver->iterationStats().back().constraint_violation_norm >
        param_epsilon_constraint.value()) {
      return false;
    }
