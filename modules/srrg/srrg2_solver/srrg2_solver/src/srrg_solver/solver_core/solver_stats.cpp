...
namespace srrg2_solver {
  ...
  std::ostream& operator<<(std::ostream& os, const IterationStats& istat) {
    os << "it= " << istat.iteration << "; level= " << istat.level
       << "; dim_variables= " << istat.dim_variables << "; dim_factors= " << istat.dim_factors
       ...
       << "; chi_kernelized= " << istat.chi_kernelized
       ...
       << "; constraint_violation= " << istat.constraint_violation_norm;
    return os;
  }
