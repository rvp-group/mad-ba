...
namespace srrg2_solver {
  ...
  template <typename VariablePtrTupleType_>
  void Factor_<VariablePtrTupleType_>::updateVariablesAccessPattern(std::vector<VariableBase::Id>& dest) const {
    for(int i=0; i<NumVariables; ++i) {
      dest.push_back(variableId(i));
    }
  }
  ...
  void Factor_<VariablePtrTupleType_>::setVariable(int pos, VariableBase* v) {
    ...
    if (v) {
      _variables.setGraphId(pos, v->graphId());
    }
  }

  template <typename VariablePtrTupleType_>
  void Factor_<VariablePtrTupleType_>::bind(IdVariablePtrContainer& container) {
    _variables.bind(container);
  }
