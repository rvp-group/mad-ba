...
namespace srrg2_solver {
  ...
  bool RobustifierHuber::robustify(srrg2_core::Vector3f& scales_, const float chi_) const {
    scales_ << chi_, 1, 0;
    const float& t = param_chi_threshold.value();
    const float residual_norm = std::sqrt(chi_);
    if (residual_norm > t) {
      scales_[0] = 2 * t * std::abs(residual_norm - t);
      scales_[1] = 2 * t / residual_norm;
      return true;
    } else {
      return false;
    }
