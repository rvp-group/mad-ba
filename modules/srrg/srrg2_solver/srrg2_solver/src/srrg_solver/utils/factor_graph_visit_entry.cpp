...
namespace srrg2_solver {
  ...
  VariableVisitEntry* VariableVisitEntryContainer::add(const VariableVisitEntry& entry) {
    // std::cerr << "VariableVisitEntryContainer::add| Adding: "
    //           << entry->variable->graphId();
    assert(!_entries.count(entry.variable->graphId()));
    //
    // std::cerr << " done" << std::endl;
    auto result = _entries.insert(std::make_pair(entry.variable->graphId(), entry));
