...
using FactorTypeChordal  = SE3PosePoseChordalEulerLeftErrorFactor;
...
TEST(DUMMY_DATA, SE3PosePoseGeodesicQuaternionErrorFactor) {
  // ia types for this test
  using VType = VariableSE3QuaternionRight;
  using FType = FactorTypeGeodesic;
...
TEST(DUMMY_DATA, SE3PosePoseChordalEulerLeftErrorFactor) {
  // ia types for this test
  using VType = VariableSE3EulerLeft;
  using FType = FactorTypeChordal;
...
// TEST(DUMMY_DATA, SE3PosePoseChordalHessianFactor) {
//   // ia types for this test
//   using VType = VariableSE3QuaternionLeft;
//   using FType = SE3PosePoseChordalHessianFactor;
...
//   // ia assert final pose is the same as the original one
//   const auto& final_estimated_pose = static_cast<VType*>(graph->variable(n_poses -
//   1))->estimate(); const auto& final_gt_pose        = gt_trajectory[n_poses - 1]; const auto diff
//   = geometry3d::t2tnq(final_estimated_pose.inverse() * final_gt_pose); ASSERT_LT(diff[0], 1e-5);
//   // ia X ASSERT_LT(diff[1], 1e-5); // ia Y ASSERT_LT(diff[2], 1e-5); // ia Z ASSERT_LT(diff[3],
//   1e-5); // ia qx ASSERT_LT(diff[4], 1e-5); // ia qy ASSERT_LT(diff[5], 1e-5); // ia qz
// }
