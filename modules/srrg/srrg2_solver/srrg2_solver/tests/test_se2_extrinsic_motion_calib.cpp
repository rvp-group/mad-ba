...
void createTrajectories(const size_t& n_poses_,
                        const Isometry2f& sensor_in_robot_,
                        Isometry2fVector& robot_trajectory_,
                        Isometry2fVector& sensor_trajectory_);

template <typename VariableType_, typename FactorType_>
void createFactorGraph(const Isometry2fVector& robot_trajectory_,
                       const Isometry2fVector& sensor_trajectory_,
...
TEST(DUMMY_DATA, SE2SensorExtrinsicPoseMotionCalibAD) {
  // ldg types for this test
  using VType = VariableSE2RightAD;
  using FType = SE2SensorExtrinsicPoseMotionCalibAD;

  // ldg create a constant offset, gt pose
  Vector3f offset_vec;
  offset_vec << 0.1f, 0.2f, 0.01f;
  const Isometry2f sensor_in_robot = geometry2d::v2t(offset_vec);

  // ldg create two trajectories with a constant offset
  Isometry2fVector robot_trajectory, sensor_trajectory;
  ...
  const auto& vec_diff = geometry2d::t2v(final_estimated_pose.inverse() * sensor_in_robot);
  LOG << "GT: " << geometry2d::t2v(sensor_in_robot).transpose() << std::endl;
  LOG << "Estimate: " << geometry2d::t2v(final_estimated_pose).transpose() << std::endl;
...
void createTrajectories(const size_t& n_poses_,
  ...
  const Isometry2f previous_sensor_pose = previous_pose * sensor_in_robot_;
  ...
  for (size_t i = 1; i < n_poses_; ++i) {
    ...
    const Isometry2f sensor_in_world = next_T * sensor_in_robot_;
...
void createFactorGraph(const Isometry2fVector& robot_trajectory_,
  ...
  offset->setEstimate(Isometry2f::Identity()); // ldg initial guess
  ...
  for (size_t i = 1; i < robot_trajectory_.size(); ++i) {
    ...
    // ldg add measurament
    Isometry2f Z_sensor = prev_sensor_pose.inverse() * curr_sensor_pose;
