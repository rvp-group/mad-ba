...
TEST(DUMMY_DATA, Sim3PosePoseErrorFactorAD) {
  ...
  ASSERT_LT(diff[0], 5e-5); //  tx
  ASSERT_LT(diff[1], 5e-5); //  ty
  ASSERT_LT(diff[2], 5e-5); //  tz
  ASSERT_LT(diff[3], 5e-5); //  rx
  ASSERT_LT(diff[4], 5e-5); //  ry
  ASSERT_LT(diff[5], 5e-5); //  rz
  ASSERT_LT(diff[6], 5e-5); //  s
}
