...
using FactorType       = SE3PosePoseChordalHessianFactor;
...
int main(int argc, char** argv) {
  ...
  ArgumentFloat std_perturbation(
    &cmd_line, "std", "std-pert", "std noisy perturbation to create initial guess", 0.01);
  cmd_line.parse();
  ...
  if (!no_guess.isSet()) {
    for (size_t i = 1; i < n_variables; ++i) {
      VariableType* v = static_cast<VariableType*>(graph->variable(i));
      Vector6f pert   = std_perturbation.value() * Vector6f::Random();
      Isometry3f iso  = v->estimate();
      v->setEstimate(geometry3d::v2t(pert) * iso);
    }
  }
...
void createFactorGraph(const Isometry3fVector& gt_trajectory_, const FactorGraphPtr& graph_) {
  ...
  for (size_t i = 1; i < gt_trajectory_.size(); ++i) {
    ...
    OmegaType odom_information  = OmegaType::Identity();
    // odom_information.block<3, 3>(3, 3) *= 100.f;
    ...
    for (const auto& id_var_pair : graph_->variables()) {
      ...
      if (delta_trans.norm() < 5.f) {
        OmegaType closure_information = OmegaType::Identity();
