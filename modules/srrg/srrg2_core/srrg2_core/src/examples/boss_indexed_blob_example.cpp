...
struct FixedSizeBlob: public BLOB{
  static constexpr int NumItems=1024;
  static constexpr int PacketSize=NumItems*sizeof(float);
  ...
  bool read(std::istream& is) override {
    is.read((char*) values, PacketSize);
    return true;
  }
  virtual void write(std::ostream& os) const override {
    os.write((const char*) values, PacketSize);
  }
  virtual int serializedSize() const { return PacketSize;}
  virtual const std::string& extension() {return _extension;}
  static const std::string _extension;
};

const std::string FixedSizeBlob::_extension("fsb");

using FixedSizeBlobReference=BLOBReference<FixedSizeBlob>;
...
void AnObject::serialize(ObjectData& data, IdContext& context) {
  ...
  ObjectData* blob_data=new ObjectData;
  _blob.serialize(*blob_data, context);
  data.setField("blob", blob_data);
}
...
void AnObject::deserialize(ObjectData& data, IdContext& context){
  ...
  ObjectData* _blob_data=dynamic_cast<ObjectData*>(data.getField("blob"));
  _blob.deserialize(*_blob_data, context);
}

// Step 3 : Register the class in the boss serialization system

BOSS_REGISTER_CLASS(AnObject)
BOSS_REGISTER_CLASS(FixedSizeBlobReference)
