...
void associateDataKernel(int numOfLeafs, int kdTreeAIdx, int kdTreeBIdx, TreeNodeTypePtr *kdTreeLeafesPtr, TreeNodeTypePtr *kdTreesPtr, SurfelMatches *matchPtr) {
...
#pragma omp parallel for 
 ...
 void DataAssociation::prepareData(std::vector<std::shared_ptr<TreeNodeType>> &kdTrees, std::vector<std::vector<TreeNodeTypePtr>> &kdTreeLeafes) {
...
// Potentially this can be parallelized, as matches contain only pairs of surfels
 ...
 void DataAssociation::prepareDataExample() {
  // int N = 1 << 20;
  // float *x, *y;
